...
let content = fs.readFileSync('src/components/club/ClubManagerDashboard.tsx', 'utf8');

content = content.replace(
  'import { Vote, Trash2, Sparkles, Users, AlertCircle, Check, ShieldAlert, X } from "lucide-react";',
  'import { Vote, Trash2, Sparkles, Users, AlertCircle, Check, ShieldAlert, X, Image as ImageIcon } from "lucide-react";\nimport { LargeFileUploader } from "../shared/LargeFileUploader.tsx";'
);
...
const oldCandidateInput = `<div className="flex gap-2">
                          <input
                            ...
                            className="w-full px-3 py-1.5 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:border-purple-500 text-xs text-zinc-900 dark:text-zinc-100"
                          />
                          <button
                            type="button"
                            onClick={() => {
...
fs.writeFileSync('src/components/club/ClubManagerDashboard.tsx', content);
console.log("Updated ClubManagerDashboard.tsx");
